...
config.cache_dimension = 300
config.total_file_number = 1000
# config.zipf_param = [0.3,0.5,0.7,0.9,1.1,1.3,1.5]
config.zipf_param = [1.15]
...
cache_hit_ratio = []
LFU_cache_hit_ratio =[]
for i in range(len(config.zipf_param)):
    config.environment = Cache_server(config.cache_dimension,config.total_file_number,config.zipf_param[i])
    AGENTS = [SAC_Discrete]
    ...
    for i in range(eval_step):
        ...
        agent_trained.conduct_action(agent_trained.action)
...
def moving_averag(x):
    ...
    y = df.rolling(window=500).mean().plot()
    plt.xlabel("file request")
    plt.ylabel("Cache hit ratio")
    plt.show()
    return y
moving_averag(lfu_reward)